// old/first Golang thing. 
// PoSWNF -> PIC payload injector.
//
// compile -> CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -ldflags "-s -w" -o f.exe kk.go
// based on -> https://github.com/odzhan/injection/blob/e635e8f8b984561a0a489bf85fd46e462c4c1b3b/wnf/wnf.c
...
func main() {
  ...
  if len(args) < 1 {
    fmt.Println("[-] no PIC file to load");
    os.Exit(1);
  }

  payload, _ := my_winapi.ReadPIC(args[1])
